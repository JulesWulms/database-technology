SELECT edgeP4."2"
FROM 
( SELECT edgeP14."2"
FROM 
( edge1x4 JOIN 
( SELECT edgeP9."1", edgeP9."2", edgeP9."4"
FROM 
( edge3x14 JOIN 
( SELECT edgeP19."1", edgeP19."2", edgeP19."3", edgeP19."4", edgeP19."14"
FROM 
( edge9x14 JOIN 
( SELECT edgeP6."1", edgeP6."2", edgeP6."3", edgeP6."4", edgeP6."9", edgeP6."14"
FROM 
( edge8x19 JOIN 
( edge8x9 JOIN 
( edge4x9 JOIN 
( edge3x9 JOIN 
( edge1x8 JOIN 
( edge1x19 JOIN 
( SELECT edgeP7."1", edgeP7."2", edgeP7."3", edgeP7."4", edgeP7."8", edgeP7."9", edgeP7."14", edgeP7."19"
FROM 
( edge3x6 JOIN 
( SELECT edgeP17."1", edgeP17."2", edgeP17."3", edgeP17."4", edgeP17."6", edgeP17."8", edgeP17."9", edgeP17."14", edgeP17."19"
FROM 
( edge2x7 JOIN 
( edge2x19 JOIN 
( edge6x19 JOIN 
( SELECT edgeP0."1", edgeP0."2", edgeP0."3", edgeP0."4", edgeP0."6", edgeP0."7", edgeP0."8", edgeP0."9", edgeP0."14", edgeP0."19"
FROM 
( edge2x17 JOIN 
( edge1x6 JOIN 
( SELECT edgeP15."1", edgeP15."2", edgeP15."3", edgeP15."4", edgeP15."6", edgeP15."7", edgeP15."8", edgeP15."9", edgeP15."14", edgeP15."17", edgeP15."19"
FROM 
( edge0x7 JOIN 
( edge2x14 JOIN 
( SELECT edgeP5."0", edgeP5."1", edgeP5."2", edgeP5."3", edgeP5."4", edgeP5."6", edgeP5."7", edgeP5."8", edgeP5."9", edgeP5."14", edgeP5."17", edgeP5."19"
FROM 
( edge11x15 JOIN 
( edge0x11 JOIN 
( edge6x15 JOIN 
( edge4x6 JOIN 
( edge7x11 JOIN 
( SELECT edgeP12."0", edgeP12."1", edgeP12."2", edgeP12."3", edgeP12."4", edgeP12."6", edgeP12."7", edgeP12."8", edgeP12."9", edgeP12."11", edgeP12."14", edgeP12."15", edgeP12."17", edgeP12."19"
FROM 
( edge5x17 JOIN 
( edge5x15 JOIN 
( SELECT edgeP18."0", edgeP18."1", edgeP18."2", edgeP18."3", edgeP18."4", edgeP18."5", edgeP18."6", edgeP18."7", edgeP18."8", edgeP18."9", edgeP18."11", edgeP18."14", edgeP18."15", edgeP18."17", edgeP18."19"
FROM 
( edge0x12 JOIN 
( edge3x12 JOIN 
( edge3x15 JOIN 
( edge0x4 JOIN 
( edge2x15 JOIN 
( edge4x17 JOIN 
( SELECT edgeP10."0", edgeP10."1", edgeP10."2", edgeP10."3", edgeP10."4", edgeP10."5", edgeP10."6", edgeP10."7", edgeP10."8", edgeP10."9", edgeP10."11", edgeP10."12", edgeP10."14", edgeP10."15", edgeP10."17", edgeP10."19"
FROM 
( edge4x18 JOIN 
( SELECT edgeP16."0", edgeP16."1", edgeP16."2", edgeP16."3", edgeP16."4", edgeP16."5", edgeP16."6", edgeP16."7", edgeP16."8", edgeP16."9", edgeP16."11", edgeP16."12", edgeP16."14", edgeP16."15", edgeP16."17", edgeP16."19", edgeP16."18"
FROM 
( edge10x17 JOIN 
( edge10x18 JOIN 
( SELECT edgeP13."0", edgeP13."1", edgeP13."2", edgeP13."3", edgeP13."4", edgeP13."5", edgeP13."6", edgeP13."7", edgeP13."8", edgeP13."9", edgeP13."10", edgeP13."11", edgeP13."12", edgeP13."14", edgeP13."15", edgeP13."17", edgeP13."19", edgeP13."18"
FROM 
( edge16x17 JOIN 
( edge6x18 JOIN 
( edge4x19 JOIN 
( edge4x5 JOIN 
( edge12x18 JOIN 
( edge4x10 JOIN 
( edge14x16 JOIN 
( edge0x10 JOIN 
( edge7x10 JOIN 
( edge17x18 JOIN 
( edge5x14 JOIN 
( edge0x14 JOIN 
( edge15x16 JOIN 
( edge8x15 JOIN 
( SELECT edge0x1."0", edge0x1."1", edge2x4."2", edge0x3."3", edge2x4."4", edge5x8."5", edge6x9."6", edge7x9."7", edge3x8."8", edge2x9."9", edge5x10."10", edge11x14."11", edge12x19."12", edge12x14."14", edge15x19."15", edge17x19."17", edge16x19."16", edge17x19."19", edge18x19."18"
FROM 
( edge5x13 JOIN 
( edge8x13 JOIN 
( edge11x12 JOIN 
( edge13x18 JOIN 
( edge11x19 JOIN 
( edge6x12 JOIN 
( edge2x12 JOIN 
( edge0x17 JOIN 
( edge8x12 JOIN 
( edge9x17 JOIN 
( edge1x3 JOIN 
( edge2x16 JOIN 
( edge2x3 JOIN 
( edge15x18 JOIN 
( edge1x9 JOIN 
( edge6x7 JOIN 
( edge10x15 JOIN 
( edge12x16 JOIN 
( edge11x17 JOIN 
( edge10x12 JOIN 
( edge0x8 JOIN 
( edge1x5 JOIN 
( edge7x14 JOIN 
( edge7x17 JOIN 
( edge3x5 JOIN 
( edge3x16 JOIN 
( edge7x9 JOIN 
( edge6x10 JOIN 
( edge1x16 JOIN 
( edge15x17 JOIN 
( edge2x8 JOIN 
( edge5x19 JOIN 
( edge3x10 JOIN 
( edge6x14 JOIN 
( edge11x13 JOIN 
( edge4x14 JOIN 
( edge4x8 JOIN 
( edge12x15 JOIN 
( edge13x15 JOIN 
( edge8x10 JOIN 
( edge9x13 JOIN 
( edge1x18 JOIN 
( edge5x10 JOIN 
( edge9x11 JOIN 
( edge4x11 JOIN 
( edge3x19 JOIN 
( edge5x16 JOIN 
( edge16x19 JOIN 
( edge14x18 JOIN 
( edge2x5 JOIN 
( edge5x8 JOIN 
( edge14x17 JOIN 
( edge1x11 JOIN 
( edge9x18 JOIN 
( edge18x19 JOIN 
( edge0x13 JOIN 
( edge3x8 JOIN 
( edge13x17 JOIN 
( edge0x3 JOIN 
( edge2x6 JOIN 
( edge2x11 JOIN 
( edge0x9 JOIN 
( edge11x14 JOIN 
( edge12x14 JOIN 
( edge12x19 JOIN 
( edge0x15 JOIN 
( edge15x19 JOIN 
( edge6x17 JOIN 
( edge6x9 JOIN 
( edge17x19 JOIN 
( edge0x1 JOIN 
( edge2x9 JOIN edge2x4
ON edge2x9."2"  = edge2x4."2"  AND edge2x9."9"  = edge2x9."9" ) 
ON edge0x1."0"  = edge0x1."0"  AND edge0x1."1"  = edge0x1."1" ) 
ON edge17x19."17"  = edge17x19."17"  AND edge17x19."19"  = edge17x19."19" ) 
ON edge6x9."6"  = edge6x9."6"  AND edge6x9."9"  = edge2x9."9" ) 
ON edge6x17."6"  = edge6x9."6"  AND edge6x17."17"  = edge17x19."17" ) 
ON edge15x19."15"  = edge15x19."15"  AND edge15x19."19"  = edge17x19."19" ) 
ON edge0x15."0"  = edge0x1."0"  AND edge0x15."15"  = edge15x19."15" ) 
ON edge12x19."12"  = edge12x19."12"  AND edge12x19."19"  = edge17x19."19" ) 
ON edge12x14."12"  = edge12x19."12"  AND edge12x14."14"  = edge12x14."14" ) 
ON edge11x14."11"  = edge11x14."11"  AND edge11x14."14"  = edge12x14."14" ) 
ON edge0x9."0"  = edge0x1."0"  AND edge0x9."9"  = edge2x9."9" ) 
ON edge2x11."2"  = edge2x4."2"  AND edge2x11."11"  = edge11x14."11" ) 
ON edge2x6."2"  = edge2x4."2"  AND edge2x6."6"  = edge6x9."6" ) 
ON edge0x3."0"  = edge0x1."0"  AND edge0x3."3"  = edge0x3."3" ) 
ON edge13x17."13"  = edge13x17."13"  AND edge13x17."17"  = edge17x19."17" ) 
ON edge3x8."3"  = edge0x3."3"  AND edge3x8."8"  = edge3x8."8" ) 
ON edge0x13."0"  = edge0x1."0"  AND edge0x13."13"  = edge13x17."13" ) 
ON edge18x19."18"  = edge18x19."18"  AND edge18x19."19"  = edge17x19."19" ) 
ON edge9x18."9"  = edge2x9."9"  AND edge9x18."18"  = edge18x19."18" ) 
ON edge1x11."1"  = edge0x1."1"  AND edge1x11."11"  = edge11x14."11" ) 
ON edge14x17."14"  = edge12x14."14"  AND edge14x17."17"  = edge17x19."17" ) 
ON edge5x8."5"  = edge5x8."5"  AND edge5x8."8"  = edge3x8."8" ) 
ON edge2x5."2"  = edge2x4."2"  AND edge2x5."5"  = edge5x8."5" ) 
ON edge14x18."14"  = edge12x14."14"  AND edge14x18."18"  = edge18x19."18" ) 
ON edge16x19."16"  = edge16x19."16"  AND edge16x19."19"  = edge17x19."19" ) 
ON edge5x16."5"  = edge5x8."5"  AND edge5x16."16"  = edge16x19."16" ) 
ON edge3x19."3"  = edge0x3."3"  AND edge3x19."19"  = edge17x19."19" ) 
ON edge4x11."4"  = edge2x4."4"  AND edge4x11."11"  = edge11x14."11" ) 
ON edge9x11."9"  = edge2x9."9"  AND edge9x11."11"  = edge11x14."11" ) 
ON edge5x10."5"  = edge5x8."5"  AND edge5x10."10"  = edge5x10."10" ) 
ON edge1x18."1"  = edge0x1."1"  AND edge1x18."18"  = edge18x19."18" ) 
ON edge9x13."9"  = edge2x9."9"  AND edge9x13."13"  = edge13x17."13" ) 
ON edge8x10."8"  = edge3x8."8"  AND edge8x10."10"  = edge5x10."10" ) 
ON edge13x15."13"  = edge13x17."13"  AND edge13x15."15"  = edge15x19."15" ) 
ON edge12x15."12"  = edge12x19."12"  AND edge12x15."15"  = edge15x19."15" ) 
ON edge4x8."4"  = edge2x4."4"  AND edge4x8."8"  = edge3x8."8" ) 
ON edge4x14."4"  = edge2x4."4"  AND edge4x14."14"  = edge12x14."14" ) 
ON edge11x13."11"  = edge11x14."11"  AND edge11x13."13"  = edge13x17."13" ) 
ON edge6x14."6"  = edge6x9."6"  AND edge6x14."14"  = edge12x14."14" ) 
ON edge3x10."3"  = edge0x3."3"  AND edge3x10."10"  = edge5x10."10" ) 
ON edge5x19."5"  = edge5x8."5"  AND edge5x19."19"  = edge17x19."19" ) 
ON edge2x8."2"  = edge2x4."2"  AND edge2x8."8"  = edge3x8."8" ) 
ON edge15x17."15"  = edge15x19."15"  AND edge15x17."17"  = edge17x19."17" ) 
ON edge1x16."1"  = edge0x1."1"  AND edge1x16."16"  = edge16x19."16" ) 
ON edge6x10."6"  = edge6x9."6"  AND edge6x10."10"  = edge5x10."10" ) 
ON edge7x9."7"  = edge7x9."7"  AND edge7x9."9"  = edge2x9."9" ) 
ON edge3x16."3"  = edge0x3."3"  AND edge3x16."16"  = edge16x19."16" ) 
ON edge3x5."3"  = edge0x3."3"  AND edge3x5."5"  = edge5x8."5" ) 
ON edge7x17."7"  = edge7x9."7"  AND edge7x17."17"  = edge17x19."17" ) 
ON edge7x14."7"  = edge7x9."7"  AND edge7x14."14"  = edge12x14."14" ) 
ON edge1x5."1"  = edge0x1."1"  AND edge1x5."5"  = edge5x8."5" ) 
ON edge0x8."0"  = edge0x1."0"  AND edge0x8."8"  = edge3x8."8" ) 
ON edge10x12."10"  = edge5x10."10"  AND edge10x12."12"  = edge12x19."12" ) 
ON edge11x17."11"  = edge11x14."11"  AND edge11x17."17"  = edge17x19."17" ) 
ON edge12x16."12"  = edge12x19."12"  AND edge12x16."16"  = edge16x19."16" ) 
ON edge10x15."10"  = edge5x10."10"  AND edge10x15."15"  = edge15x19."15" ) 
ON edge6x7."6"  = edge6x9."6"  AND edge6x7."7"  = edge7x9."7" ) 
ON edge1x9."1"  = edge0x1."1"  AND edge1x9."9"  = edge2x9."9" ) 
ON edge15x18."15"  = edge15x19."15"  AND edge15x18."18"  = edge18x19."18" ) 
ON edge2x3."2"  = edge2x4."2"  AND edge2x3."3"  = edge0x3."3" ) 
ON edge2x16."2"  = edge2x4."2"  AND edge2x16."16"  = edge16x19."16" ) 
ON edge1x3."1"  = edge0x1."1"  AND edge1x3."3"  = edge0x3."3" ) 
ON edge9x17."9"  = edge2x9."9"  AND edge9x17."17"  = edge17x19."17" ) 
ON edge8x12."8"  = edge3x8."8"  AND edge8x12."12"  = edge12x19."12" ) 
ON edge0x17."0"  = edge0x1."0"  AND edge0x17."17"  = edge17x19."17" ) 
ON edge2x12."2"  = edge2x4."2"  AND edge2x12."12"  = edge12x19."12" ) 
ON edge6x12."6"  = edge6x9."6"  AND edge6x12."12"  = edge12x19."12" ) 
ON edge11x19."11"  = edge11x14."11"  AND edge11x19."19"  = edge17x19."19" ) 
ON edge13x18."13"  = edge13x17."13"  AND edge13x18."18"  = edge18x19."18" ) 
ON edge11x12."11"  = edge11x14."11"  AND edge11x12."12"  = edge12x19."12" ) 
ON edge8x13."8"  = edge3x8."8"  AND edge8x13."13"  = edge13x17."13" ) 
ON edge5x13."5"  = edge5x8."5"  AND edge5x13."13"  = edge13x17."13" ) 
WHERE (1=1) ) AS edgeP13
ON edge8x15."8"  = edgeP13."8"  AND edge8x15."15"  = edgeP13."15" ) 
ON edge15x16."15"  = edgeP13."15"  AND edge15x16."16"  = edgeP13."16" ) 
ON edge0x14."0"  = edgeP13."0"  AND edge0x14."14"  = edgeP13."14" ) 
ON edge5x14."5"  = edgeP13."5"  AND edge5x14."14"  = edgeP13."14" ) 
ON edge17x18."17"  = edgeP13."17"  AND edge17x18."18"  = edgeP13."18" ) 
ON edge7x10."7"  = edgeP13."7"  AND edge7x10."10"  = edgeP13."10" ) 
ON edge0x10."0"  = edgeP13."0"  AND edge0x10."10"  = edgeP13."10" ) 
ON edge14x16."14"  = edgeP13."14"  AND edge14x16."16"  = edgeP13."16" ) 
ON edge4x10."4"  = edgeP13."4"  AND edge4x10."10"  = edgeP13."10" ) 
ON edge12x18."12"  = edgeP13."12"  AND edge12x18."18"  = edgeP13."18" ) 
ON edge4x5."4"  = edgeP13."4"  AND edge4x5."5"  = edgeP13."5" ) 
ON edge4x19."4"  = edgeP13."4"  AND edge4x19."19"  = edgeP13."19" ) 
ON edge6x18."6"  = edgeP13."6"  AND edge6x18."18"  = edgeP13."18" ) 
ON edge16x17."16"  = edgeP13."16"  AND edge16x17."17"  = edgeP13."17" ) 
WHERE (1=1) ) AS edgeP16
ON edge10x18."10"  = edgeP16."10"  AND edge10x18."18"  = edgeP16."18" ) 
ON edge10x17."10"  = edgeP16."10"  AND edge10x17."17"  = edgeP16."17" ) 
WHERE (1=1) ) AS edgeP10
ON edge4x18."4"  = edgeP10."4"  AND edge4x18."18"  = edgeP10."18" ) 
WHERE (1=1) ) AS edgeP18
ON edge4x17."4"  = edgeP18."4"  AND edge4x17."17"  = edgeP18."17" ) 
ON edge2x15."2"  = edgeP18."2"  AND edge2x15."15"  = edgeP18."15" ) 
ON edge0x4."0"  = edgeP18."0"  AND edge0x4."4"  = edgeP18."4" ) 
ON edge3x15."3"  = edgeP18."3"  AND edge3x15."15"  = edgeP18."15" ) 
ON edge3x12."3"  = edgeP18."3"  AND edge3x12."12"  = edgeP18."12" ) 
ON edge0x12."0"  = edgeP18."0"  AND edge0x12."12"  = edgeP18."12" ) 
WHERE (1=1) ) AS edgeP12
ON edge5x15."5"  = edgeP12."5"  AND edge5x15."15"  = edgeP12."15" ) 
ON edge5x17."5"  = edgeP12."5"  AND edge5x17."17"  = edgeP12."17" ) 
WHERE (1=1) ) AS edgeP5
ON edge7x11."7"  = edgeP5."7"  AND edge7x11."11"  = edgeP5."11" ) 
ON edge4x6."4"  = edgeP5."4"  AND edge4x6."6"  = edgeP5."6" ) 
ON edge6x15."6"  = edgeP5."6"  AND edge6x15."15"  = edgeP5."15" ) 
ON edge0x11."0"  = edgeP5."0"  AND edge0x11."11"  = edgeP5."11" ) 
ON edge11x15."11"  = edgeP5."11"  AND edge11x15."15"  = edgeP5."15" ) 
WHERE (1=1) ) AS edgeP15
ON edge2x14."2"  = edgeP15."2"  AND edge2x14."14"  = edgeP15."14" ) 
ON edge0x7."0"  = edgeP15."0"  AND edge0x7."7"  = edgeP15."7" ) 
WHERE (1=1) ) AS edgeP0
ON edge1x6."1"  = edgeP0."1"  AND edge1x6."6"  = edgeP0."6" ) 
ON edge2x17."2"  = edgeP0."2"  AND edge2x17."17"  = edgeP0."17" ) 
WHERE (1=1) ) AS edgeP17
ON edge6x19."6"  = edgeP17."6"  AND edge6x19."19"  = edgeP17."19" ) 
ON edge2x19."2"  = edgeP17."2"  AND edge2x19."19"  = edgeP17."19" ) 
ON edge2x7."2"  = edgeP17."2"  AND edge2x7."7"  = edgeP17."7" ) 
WHERE (1=1) ) AS edgeP7
ON edge3x6."3"  = edgeP7."3"  AND edge3x6."6"  = edgeP7."6" ) 
WHERE (1=1) ) AS edgeP6
ON edge1x19."1"  = edgeP6."1"  AND edge1x19."19"  = edgeP6."19" ) 
ON edge1x8."1"  = edgeP6."1"  AND edge1x8."8"  = edgeP6."8" ) 
ON edge3x9."3"  = edgeP6."3"  AND edge3x9."9"  = edgeP6."9" ) 
ON edge4x9."4"  = edgeP6."4"  AND edge4x9."9"  = edgeP6."9" ) 
ON edge8x9."8"  = edgeP6."8"  AND edge8x9."9"  = edgeP6."9" ) 
ON edge8x19."8"  = edgeP6."8"  AND edge8x19."19"  = edgeP6."19" ) 
WHERE (1=1) ) AS edgeP19
ON edge9x14."9"  = edgeP19."9"  AND edge9x14."14"  = edgeP19."14" ) 
WHERE (1=1) ) AS edgeP9
ON edge3x14."3"  = edgeP9."3"  AND edge3x14."14"  = edgeP9."14" ) 
WHERE (1=1) ) AS edgeP14
ON edge1x4."1"  = edgeP14."1"  AND edge1x4."4"  = edgeP14."4" ) 
WHERE (1=1) ) AS edgeP4
WHERE (1=1)
